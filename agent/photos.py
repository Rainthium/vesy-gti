...
class PhotoLibrary:
    ...
    def _local_thumb(self, original: Path) -> bytes | None:
        ...
        try:
            # через временный файл: убитая посреди записи служба иначе
            # оставила бы обрезанную картинку (как в центре)
            tmp = target.with_name(target.name + ".part")
            tmp.write_bytes(data)
            tmp.replace(target)
        except OSError as exc:
            logger.warning("миниатюра %s не сохранена: %s", target, exc)
        return data

    def _from_center(self, weighing_uuid: UUID, role: CameraRole, *, thumb: bool) -> bytes | None:
