...
class ScaleSettingsPayload(BaseModel):
    ...
    # ручной режим при ЖИВОЙ связи с центром (агент 0.4.28; решение Игоря
    # 03.09.2026 — объект, к которому АИС «СВХ» ещё не подключена): True —
    # кнопки взвешивания/тарирования доступны оператору и при связи, записи
    # идут как local_offline и досылаются сразу; False — правило №3 как
    # прежде; None — не задано / агенты до 0.4.28 отбрасывают
