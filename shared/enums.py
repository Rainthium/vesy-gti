...
class WeighingSource(StrEnum):
    """Источник операции: команда АИС «СВХ», ручной офлайн-режим оператора
    или перенос из АИС при переключении объекта на весовую систему."""

    AIS = "ais"
    LOCAL_OFFLINE = "local_offline"
    # тарирование, проведённое через UniServer до переключения объекта и
    # перенесённое из выгрузки АИС «СВХ» (решение Игоря 12.09.2026): нашей
    # операции за ним нет — ни снимков, ни карточки, только тара для нетто;
    # в отчёты, API v2 и события не попадает
    IMPORTED = "imported"
