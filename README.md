...
| `docs/install-agent-windows.md` | Установка агента на весовой ПК (в т.ч. второй агент на том же ПК) |
...
│               # cameras/ (снапшоты + постоянные RTSP-потоки с буфером кадра),
│               # web/ (интерфейс оператора), sync/ (SQLite, WS-клиент,
│               # очередь фото, ретеншн), photos.py (снимки журнала), diagnostics.py
