...
class TestReportPage:
    def test_page_shows_seed_numbers(self, env: ReportsEnv) -> None:
        ...
        # пояснение красной пилюли показывается, только когда такие случаи есть
        assert ("«Тара была, но не подставилась»: тарирование действовало" in page) == (
            "тара была, но не подставилась: 0" not in page
        )
