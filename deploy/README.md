...
## 9а. Каталог инструментов для агентов: ffmpeg по надобности (0.4.33)

Центр раздаёт агентам не только релизы, но и инструменты — сегодня это
`ffmpeg.exe` для камер, отдающих кадр только по RTSP (урок Канта 12.09.2026:
ffmpeg в мастер-папке не было, зайти на ПК нельзя, а класть 100 МБ в каждый
релиз — раздавать его всем объектам без нужды). Каталог
`~/vesy-gti/deploy/tools` (том `/data/tools` контейнера, только чтение,
`AGENT_TOOLS_DIR`), маршрут `GET /agents/tools/<файл>` — только с токеном
агента, контрольная сумма и размер уходят заголовками `X-Sha256` /
`X-Size-Bytes` (center/tools.py, center/tools_router.py). Файлы кладутся scp
с рабочей машины; `ship.sh` каталог не трогает (исключён из rsync — иначе
`--delete` снёс бы его). Рабочая копия ffmpeg — та же сборка
ffmpeg-release-essentials (gyan.dev, 12.08.2026), что стоит на Джалал-Абаде:

```bash
ssh vesy@192.168.140.70 'mkdir -p ~/vesy-gti/deploy/tools'
scp ../ves-agent-jalalabad/ffmpeg.exe ../ves-agent-jalalabad/FFMPEG-LICENSE.txt vesy@192.168.140.70:~/vesy-gti/deploy/tools/
```

Агент 0.4.33+ качает `ffmpeg.exe` сам и только по надобности: когда в его
настройках (панель → «Справочники» → камеры) появляется камера только с RTSP,
а ffmpeg на ПК не найден (agent/ffmpeg_tool.py). Файл кладётся в корень
установки (`D:\vesy-agent\ffmpeg.exe`, рядом с `config.toml`), обновления его
не трогают; потоки поднимаются без рестарта службы, в журнале агента —
«ffmpeg получен с центра», в логе центра — «агент весов N скачивает
инструмент ffmpeg.exe». Объекты с камерами-снимками ничего не скачивают.
Проверка раздачи с ВМ: `curl -sI -H 'Authorization: Bearer <токен агента>'
http://localhost/agents/tools/ffmpeg.exe` → `200`, заголовок `X-Sha256`
совпадает с `sha256sum` файла в каталоге (токен — из файла, не из истории
команд).

## 9б. Перенос тарирований из АИС «СВХ» при переключении объекта (разово, 12.09.2026)

Когда объект переходит с UniServer на весовую систему, действующие тары его
сцепок живут только в АИС «СВХ» (тарирования шли через UniServer). Чтобы
машинам не тарироваться заново, их переносят из выгрузки «Тарирования» АИС
(CSV, разделитель «;», UTF-8 с BOM, время бишкекское) консольным инструментом
в контейнере центра; кнопки в панели нет (решение Игоря 12.09.2026, decisions):

```bash
scp "Тарирования в АИС СВХ.csv" vesy@192.168.140.70:/tmp/tares.csv
ssh vesy@192.168.140.70
cd ~/vesy-gti/deploy
docker compose exec -T postgres pg_dump -U ves -Fc ves > ~/backups/ves-$(date +%F).dump  # бэкап
docker cp /tmp/tares.csv ves-center-app-1:/tmp/tares.csv
# проверка без записи: вердикт каждой строке, сводка по объектам, построчный отчёт
docker compose exec -T app uv run --no-sync python -m tools.import_ais_tares /tmp/tares.csv \
    --report /tmp/tares-report.csv
# сам перенос
docker compose exec -T app uv run --no-sync python -m tools.import_ais_tares /tmp/tares.csv --apply
docker cp ves-center-app-1:/tmp/tares-report.csv ./  # построчный отчёт, при желании
```

Вердикты: к переносу / старше 3 месяцев / уже в журнале центра (по номеру TAR
из `weighing_ais_refs` либо по совпадению объекта, номера ТС, массы ±20 кг и
времени ±30 мин) / уже перенесено раньше / незавершённое / помечено «не
учитывать при взвешивании» / без номера ТС / тяжелее лимита (25 т,
`--max-tare-kg`) / объект не заведён. С `--apply` записи ложатся в журнал с
источником «Перенос из АИС» (без снимков и карточки печати), номер TAR
закрепляется, реестр тар обновляется, пишется аудит `tares_import` — одной
транзакцией. Повтор безопасен: перенесённое узнаётся по номеру TAR, поэтому
после переключения объекта на центр берут свежую выгрузку и запускают ещё
раз — войдут только тарирования за дни между первым переносом и
переключением. Объекты выгрузки сопоставлены с весами центра таблицей
`AIS_OBJECTS` в инструменте (у Канта тарируют на «Весы №2 (тарирование)»);
новый объект — дописать в таблицу или передать
`--object "Имя в АИС=код_объекта[/имя весов]"`.

Реестр агентам инструмент не рассылает (он вне процесса центра): снимок уйдёт
при следующем тарировании на любом объекте либо при переподключении агента;
если нужно немедленно — `docker compose restart app` (20 с, агенты
переподключатся и получат полный снимок при hello). Перенесённые записи не
попадают в отчёты, API v2 и события АИС (decisions 12.09.2026).
